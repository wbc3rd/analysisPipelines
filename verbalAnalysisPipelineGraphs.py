#verbalAnalysisPipelineGraphs reads in the stimLists from a subjects session and checks the words against 
#the subject provided recall list. this version generates pie charts to illustrate
#subject performance
...
practicePercent = (totalPracticeWords/totalWords) * 100
print(practicePercent)

print("Words from the practice session: ")
print(subjectNewPCorrect + subjectOldPCorrect + subjectNewPWrong + subjectOldPWrong)

colors = ['green', 'red']
y = np.array([percentCorrect, percentWrong]) 
label1 = 'Percent Correct ' + str(round(percentCorrect, 2)) + '%'
label2 = 'Percent Incorrect ' + str(round(percentWrong, 2)) + '%'
mylabels = [label1, label2]

plt.pie(y, labels = mylabels, colors=colors)
plt.title(cobraTag + runDate + ' Verbal Recall Performance')
plt.show()